...
actor SemanticExtractor {
    ...
    func extract(from text: String, now: Date = .now) async -> ExtractionOutput {
        ...
        2. memos: 先輩・上司などから教わった仕事のやり方、具体的な手順、ルール、注意点、レクチャー内容だけ。

        重要ルール:
        - 雑談、感想、世間話、挨拶、相づち、無音由来の不自然な文字列は保存しない。
        - この処理は選別だけを行う。文章の生成、要約、言い換え、補足、推測は禁止。
        - TodoのtitleとMemoのbodyは、入力の会話からそのまま抜き出した文字列だけにする。
        - 入力に明示されていない行動・知識・手順は絶対に作らない。
        - 判断に迷う場合はtodosもmemosも空にする。
        - 「教わった」「教えてもらった」「先輩が言っていた」「上司からの指示」など、他人からのレクチャーはメモ対象。
        - 単なる予定・行動・感想・雑談はメモにしない。
